...
class machinelearning():
    ...
    def train(self, traindata, testdata, randomnum):
        ...
        for i in tqdm.tqdm(range(randomnum)):
            ...
            train_x = np.concatenate((sd_trainx, rna_trainx, prot_trainx),axis=1)
            val_x = np.concatenate((sd_valx, rna_valx, prot_valx),axis=1)
            newtestx = np.concatenate((sd_testx, rna_testx, prot_testx),axis=1)
            ...
            model = keras.Model([inputs2, inputs3, inputs4], out)
            ...
            model.fit([rna_trainx, prot_trainx, sd_trainx], train_y, batch_size=64, epochs=100, verbose=1,
                      validation_data=([rna_valx, prot_valx, sd_valx], val_y), callbacks=callbacks_list)
            nn = keras.models.load_model(args.modeldir+'/nn.h5')

            predict = np.array(nn.predict([rna_valx, prot_valx, sd_valx]))
            predict[predict >= 0.5] = 1
            predict[predict < 0.5] = 0
            nnauc = roc_auc_score(val_y, nn.predict([rna_valx, prot_valx, sd_valx]))
            ...
            nntesty = nn.predict([rna_testx, prot_testx, sd_testx])
