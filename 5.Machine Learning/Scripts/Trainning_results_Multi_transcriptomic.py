...
class machinelearning():
    ...
    def train(self, traindata, testdata, randomnum):
        ...
        for i in tqdm.tqdm(range(randomnum)):
            ...
            train_x = np.concatenate((sd_trainx, trans_trainx),axis=1)
            val_x = np.concatenate((sd_valx, trans_valx),axis=1)
            newtestx = np.concatenate((sd_testx, trans_testx),axis=1)
            ...
            model = keras.Model([inputs1, inputs4], out)
            ...
            model.fit([trans_trainx, sd_trainx], train_y, batch_size=64, epochs=100, verbose=1,
                      validation_data=([trans_valx, sd_valx], val_y), callbacks=callbacks_list)
            nn = keras.models.load_model(args.modeldir+'/nn.h5')

            predict = np.array(nn.predict([trans_valx, sd_valx]))
            predict[predict >= 0.5] = 1
            predict[predict < 0.5] = 0
            nnauc = roc_auc_score(val_y, nn.predict([trans_valx, sd_valx]))
            ...
            nntesty = nn.predict([trans_testx, sd_testx])
